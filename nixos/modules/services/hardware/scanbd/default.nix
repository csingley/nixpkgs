...
let
  ...
  utils = rec {
    ...
    renderBlock = { name, content, indent ? 0 }:
      let
    ...
    renderAction = { name, attrs, trigger, indent ? 2 }:
      let
    ...
    renderDevice = { name, filter, desc ? "", actions ? {}, indent ? 2 }:
      let
        # Render device attributes
        deviceAttrs = renderAttrSet {
          attrs = { inherit filter; } // lib.optionalAttrs (desc != "") { inherit desc; };
          indent = indent + 2;
        };

        # Render device-specific actions
        deviceActions = lib.concatMapAttrsStringSep "\n\n"
          (actionName: actionCfg: renderAction {
            name = actionName;
        ...
        deviceContent = lib.concatStringsSep "\n\n" (
          [ deviceAttrs ]
          ++ lib.optional (actions != {}) deviceActions
        );
      in
        renderBlock {
          name = "device ${name}";
  ...
  processedActions = lib.mapAttrs (name: actionCfg:
    if name == "scan" && actionCfg.script == null
    then actionCfg // {
      script = pkgs.runCommand "scanbd-scan-script" {} ''
        install -Dm755 ${pkgs.replaceVars ./scanbd-builtin-scan-script.sh {
          # Package paths
          ...
          scannerBinary =
            if actionCfg.scannerBinary == "scanadf"
            then "${pkgs.sane-frontends}/bin/scanadf"
            else actionCfg.scannerBinary;
        }} $out
      '';
    }
    else actionCfg
...
saned_env = {
  ...
  renderedActions = lib.concatMapAttrsStringSep "\n\n"
    (name: actionCfg: utils.renderAction {
      inherit name;
      attrs = {
  ...
  renderedDevices = lib.concatMapAttrsStringSep "\n\n"
    (name: deviceCfg: utils.renderDevice {
      inherit name;
...

{
        ...
        options = {
          ...
          trigger = lib.mkOption {
            type = lib.types.submodule {
              options = {
                type = lib.mkOption {
                  type = lib.types.enum [ "numerical-trigger" "string-trigger" ];
  ...
  config = lib.mkIf cfg.enable {
    ...
    assertions = [
      ...
      {
        assertion = !(cfg.actions ? scan) || cfg.actions.scan.script != null || cfg.actions.scan.outputDirectory != "";
